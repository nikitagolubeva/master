...
class WPFTS_Flare
{
	...
	public function SendFire($data = array(), $is_update = false, $is_blocking = false)
	{
		...
		$packet = array(
			...
			'body' => wpfts_json_encode(array(
				'mid' => $this->mid,
				'ts' => time(), // Current actual timestamp
				't' => $is_update ? 'u' : 'f', // Full frame
				'sd' => home_url(),
				'si' => isset($_SERVER['SERVER_ADDR']) ? $_SERVER['SERVER_ADDR'] : (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'no_server_addr'),
				'msg' => $data,
			)),
	...
	{
		$mid = md5(uniqid('flare_mid')).('|'.home_url().'|'.(isset($_SERVER['SERVER_ADDR']) ? $_SERVER['SERVER_ADDR'] : (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'no_server_addr')));
