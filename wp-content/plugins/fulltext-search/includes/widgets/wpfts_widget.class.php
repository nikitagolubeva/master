...
class WPFTS_Custom_Widget extends WP_Widget
{
	...
	public function widget($args, $instance)
	{
		...
		?><div class="wpfts_widget wpfts_search_widget presetid-<?php echo esc_attr($preset_id2); ?><?php echo strlen($class) > 0 ? ' '.esc_attr($class) : ''; ?>">
				...
				?>
		<form role="search" class="wpfts_search_form-<?php echo esc_attr($preset_id2); ?> search-form <?php echo esc_attr($preset['autocomplete_mode'] ? ' wpfts_autocomplete': ''); ?>" action="<?php echo esc_url($preset['results_url']); ?>" method="get">
			<?php echo (strlen($preset_id) > 0) ? '<input type="hidden" name="wpfts_wdgt" value="'.esc_attr($preset_id).'">' : ''; ?>
			<label>
				<span class="screen-reader-text"><?php echo esc_html(__('Search for:', 'fulltext-search')); ?></span>
	...
	public function form($instance)
	{
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>">
				<?php echo esc_html(__('Title:', 'fulltext-search')); ?> 
		...
		<label for="<?php echo esc_attr($wpfts_wdgt_id); ?>">
				<?php echo esc_html(__('Preset:', 'fulltext-search')); ?>
				<a href="#" id="<?php echo esc_attr($wpfts_wdgt_id).'_edtpst'; ?>" target="_blank"><?php echo esc_html(__('Edit Presets', 'fulltext-search')); ?></a> 
					...
					<?php
						foreach ($widget_list as $k => $d) {
							echo '<option value="'.esc_attr($k).'"'.($k == $wpfts_wdgt ? ' selected="selected"' : '').'>'.esc_html($d).'</option>';
						}
					?>
				</select>
			</label>
		...
		<label for="<?php echo esc_attr($this->get_field_id('placeholder')); ?>">
				<?php echo esc_html(__('Placeholder Text:', 'fulltext-search')); ?> 
		...
		<label for="<?php echo esc_attr($this->get_field_id('button_text')); ?>">
				<?php echo esc_html(__('Button Text:', 'fulltext-search')); ?> 
			...
			<label for="<?php echo esc_attr($this->get_field_id('hide_button')); ?>">
				...
				<?php echo esc_html(__('Hide Button', 'fulltext-search')); ?> 
			</label>
		</p>

		<p>
		<label for="<?php echo esc_attr($this->get_field_id('class')); ?>">
				<?php echo esc_html(__('CSS Class:', 'fulltext-search')); ?> 
