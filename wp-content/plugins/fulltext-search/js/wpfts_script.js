...
var flare_connect_status = 0;
var last_flare_connect_try_dt = 0;
...
jQuery(document).ready(function()
{
	...
	jQuery(document).on('click', '.wpfts_btn_rebuild', function(e)
	{
        e.preventDefault();
	...
	jQuery('.btn_se_style_reset').on('click', function(e)
	{
		...
		var nonce = jQuery('#wpftsi_form5 input[name="_nonce"]').val();
		wpftsiAction('wpftsi_se_style_reset', {'_nonce': nonce}, function(jx) 
		{
            if (('code' in jx) && (jx.code == 0) && ('c_css' in jx)) {
                jQuery('#wpfts_se_styles_node').html(jx.c_css);
                if (wpfts_se_styles_editor) {
	...
	jQuery(document).on('click', '.wpfts_test_index_table .wpfts_btn_preview_change', function(e)
	{
		e.preventDefault();

		var tr = jQuery(this).closest('tr');

		if (jQuery('.wpfts_set_preview', jQuery(this)).is(':visible')) {
			jQuery('.wpfts_set_preview', jQuery(this)).css('display', 'none');
			jQuery('.wpfts_set_showjson', jQuery(this)).css('display', 'inline');
			jQuery('.wpfts_display_column .wpfts_show_json', tr).css('display', 'none');
			jQuery('.wpfts_display_column .wpfts_show_preview', tr).css('display', 'inline');
		} else {
			jQuery('.wpfts_set_preview', jQuery(this)).css('display', 'inline');
			jQuery('.wpfts_set_showjson', jQuery(this)).css('display', 'none');
			jQuery('.wpfts_display_column .wpfts_show_json', tr).css('display', 'inline');
			jQuery('.wpfts_display_column .wpfts_show_preview', tr).css('display', 'none');
		}

		return false;
	});

	jQuery('.wpfts_btn_upgrade_index').on('click', function(e)
	{
		e.preventDefault();
	...
	jQuery('.btn_wpfts_add_user_irule').on('click', function(e){
		e.preventDefault();

		wpftsiAction('wpftsi_add_user_irule', {}, function(jx)
		{	
			if (('code' in jx) && (jx['code'] === 0)) {
				//
			}
	...
	fc.addHandler('connect', function(status)
	{
		flare_connect_status = status;
	});
	...
	var pingprocessor = function(jx, callback = false) 
	{
        if (('code' in jx) && (jx['code'] === 0)) {
			if ('status' in jx) {
				wpftsShowIndexStatus(jx['status']);
    ...
    //if ((typeof wpfts_is_settings_screen != 'undefined') && (wpfts_is_settings_screen)) {
		...
		{
			if (flare_connect_status == 1) {
				// Flare is connected, let's use Flare information only, avoid to overload the server
				// by frequent requests
				setTimeout(pingtimer, 15000);	// Repeat this check each 15 seconds
			} else {
				// Flare is not connected, let's try to reconnect (once per minute)
				if (last_flare_connect_try_dt + 60000 < (new Date()).getTime()) {
					// Go reconnect
					last_flare_connect_try_dt = (new Date()).getTime();
				...
				// Flare is not connected, let's call WP ajax to retrieve any new status data
				var post_ids = [];
				...
				wpftsiAction('wpftsi_ping', {'box_post_ids': post_ids}, function(jx) 
				{
					pingprocessor(jx, function()
					{
						// Start new timeout
						setTimeout(pingtimer, 15000);	// Ping each 15 seconds
					});
				});
			}
		};

		// Do not ping immediately, wait 10 seconds
		setTimeout(pingtimer, 10000);
    //}
